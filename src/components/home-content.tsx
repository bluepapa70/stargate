          ...
          <div className="grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
            <form
              ...
              onSubmit={(event) => {
                ...
                router.push(`/horoscope/${dateKey}/${sign.key}?birthDate=${birthDate}`);
              }}
            >
              ...
              <p className="mt-2 text-xs leading-6 text-[#cba98e]">
                운세 데이터 기준일: {dateKey}
                {hasOverrides ? " · 날짜별 데이터 적용 중" : " · 기본 템플릿 사용 중"}
              </p>
            </form>
        ...
        <aside className="rounded-[2rem] border border-line bg-panel px-5 py-5 shadow-[0_18px_50px_rgba(80,20,180,0.18)] sm:px-6">
          ...
          <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
            {zodiacSigns.map((sign, index) => (
              <Link
                key={sign.key}
                href={`/horoscope/${dateKey}/${sign.key}?birthDate=${sampleBirthDates[index]}`}
